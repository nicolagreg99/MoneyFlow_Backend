...
def total_balances_by_month():
    try:
        ...
        for i in range(12):
            ...
            result[f"{mesi_nomi[mese_riferimento]} {anno_riferimento}"] = 0.00  # Default a 0.00

        # Inseriamo i dati delle entrate nei risultati
        for anno, mese, totale_entrate in totali_entrate:
            chiave = f"{mesi_nomi[mese]} {anno}"
            result[chiave] += round(float(totale_entrate), 2)  # Aggiungiamo le entrate

        # Sottraiamo le spese dai risultati
        for anno, mese, totale_spese in totali_spese:
            chiave = f"{mesi_nomi[mese]} {anno}"
            result[chiave] -= round(float(totale_spese), 2)  # Sottraiamo le spese

        # Formattiamo i valori con due cifre decimali
        result_formattato = {k: round(v, 2) for k, v in result.items()}

        return jsonify(result_formattato), 200
